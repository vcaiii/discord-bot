export function getMusic(client) {
  const channel = client.channels.cache.find(chan => chan.name.match(/music$/)) // Designated Channel
  const musicLinks = grabLinks(channel)
  return musicLinks
}
